...
Then the association measures are calculated for every row:

```{r}
measured <- calculateAssociation(integrated)
kable(measured[sample(1:nrow(measured),6),])
```

Finally the dataframe is reformated for convenient access by pair (target concept, related concept):

```{r}
lbd_data<-reformatRelationsByTarget(measured, targetPairs)
...
```{r}
write.table(finalData, 'lbd_data.tsv',sep='\t',quote = FALSE,row.names=FALSE)
```
